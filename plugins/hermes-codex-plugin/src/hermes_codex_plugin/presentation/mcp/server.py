...
from hermes_codex_plugin.domain.skills.services import normalize_skill_name
...
class MCPServer:
    def __init__(self) -> None:
        ...
        self.store = SQLiteMemoryRepository(self.settings.db_path)
        memory_mapper = MemoryEntryMapper()
        stats_mapper = MemoryStatsMapper()
        self._skill_mapper = SkillDraftMapper()
        self._search = SearchMemoryHandler(self.store, memory_mapper)
        self._search_chats = SearchChatsHandler(self.store, memory_mapper)
        self._remember = RememberMemoryHandler(self.store)
        self._forget = ForgetMemoryHandler(self.store)
        self._stats = GetMemoryStatsHandler(self.store, stats_mapper)
        self._propose_skill = ProposeSkillHandler(self.store)
    ...
    def handle_message(self, message: JSONDict) -> Any:
        ...
        if method == "tools/call":
            ...
            if tool_name not in self.tools:
                return self.error(request_id, -32601, "Unknown tool {}".format(tool_name))
    ...
    def tool_search(self, args: JSONDict) -> str:
        query = str(args.get("query") or "")
        limit = int(args.get("limit") or self.settings.recall_limit)
        cwd = args.get("cwd")
        results = self._search(SearchMemory(query, limit=limit, cwd=str(cwd) if cwd else None))
        return format_search_results(results)

    def tool_search_chats(self, args: JSONDict) -> str:
        query = str(args.get("query") or "")
        limit = int(args.get("limit") or self.settings.recall_limit)
        results = self._search_chats(SearchChats(query, limit=limit))
        return format_search_results(results)

    def tool_remember(self, args: JSONDict) -> str:
        content = str(args.get("content") or "").strip()
        if not content:
            raise ValueError("content is required")
        entry_id = self._remember(
            RememberMemory(
                content,
    ...
    def tool_forget(self, args: JSONDict) -> str:
        entry_id = int(args.get("id"))
        return (
            "Deleted entry #{}".format(entry_id)
            if self._forget(ForgetMemory(entry_id))
            else "Entry not found"
        )

    def tool_stats(self, args: JSONDict) -> str:
        del args
        return json.dumps(dataclasses.asdict(self._stats()), indent=2, sort_keys=True)

    def tool_propose_skill(self, args: JSONDict) -> str:
        draft = self._propose_skill(
            ProposeSkill(
                query=str(args.get("query") or ""),
    ...
    def tool_write_skill(self, args: JSONDict) -> str:
        name = normalize_skill_name(str(args.get("name") or "learned-workflow"))
        description = str(args.get("description") or "Use when Codex should apply learned workflow rules.")
    ...
    return [
        {
            ...
            "description": "Search local Codex memory with SQLite full-text search.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "query": {"type": "string"},
        ...
        },
        {
            ...
            "description": "Show local memory database statistics.",
            "inputSchema": {"type": "object", "properties": {}},
        },
        {
            "name": "hermes_codex_propose_skill",
            "description": "Draft a SKILL.md from matching or recent memory rules.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "query": {"type": "string"},
